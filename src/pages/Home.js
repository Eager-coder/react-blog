...
function Home() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            const { data: {body: [{ primary: {left_link, top_link, bottom_link} }]} } = await Client.getSingle('home_page')
            ...
            setLatests(results)
            results.forEach(e => {
                if (e.uid === left_link.uid ) setLeftLink({
                    uid: e.uid,
                ...
                if (e.uid === top_link.uid) setTopLink({
                    uid: e.uid,
                    image: e.data.image.url,
                    title: e.data.title[0].text,
                ...
                if (e.uid === bottom_link.uid) setBottomLink({
                    uid: e.uid,
                    image: e.data.image.url,
                    title: e.data.title[0].text,
                    // desc: e.data.body[0].primary.text[0].text.slice(0, 80),
